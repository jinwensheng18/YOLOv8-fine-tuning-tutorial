...
def get_full_class_names():
    ...
    with open(VAL_DATA_YAML, encoding="utf-8") as f:
...
# 4. 合并对比 & 计算变化率
df_merge = pd.merge(df_official, df_custom, on=["class_id", "class_name"], suffixes=("_official", "_custom"))
...
df_merge["recall_change(%)"] = df_merge.apply(
    lambda x: "新增"
    if x["recall_official"] == 0
    else round((x["recall_custom"] - x["recall_official"]) / x["recall_official"] * 100, 2),
    axis=1,
)
